...
$versionHistoryPath = "C:\ProgramData\Remote_Auto\version_history_run_for_all.json"
...
if ($protocolScripts -in @("ftp", "ftps")) {
    try {
        ...
        $versions = $files | ForEach-Object { 
            if ($_ -match '^run_for_all_v(\d+)\.ps1$') { [int]$matches[1] } 
        }
        $streamReader.Close()
        $response.Close()
    }
    ...
    try {
        ...
        $files = Get-ChildItem "${driveName}:\" | Where-Object { 
            $_.Name -match '^run_for_all_v\d+\.ps1$'
        }
        $versions = $files | ForEach-Object { 
            if ($_.Name -match '^run_for_all_v(\d+)\.ps1$') { [int]$matches[1] } 
        }
    }
    catch {
...
if (-not $versions) {
    Write-Error "Не найдено подходящих скриптов"
    exit 1
}
...
if ($protocolScripts -in @("ftp", "ftps")) {
    ...
    try {
        ...
        $remoteScript = "${driveName}:\run_for_all_v$targetVersion.ps1"
...
# ===== ВЫПОЛНЕНИЕ СКРИПТА =====
$logName = "run_for_all_${computerName}_v${targetVersion}_$(Get-Date -Format 'yyyyMMddHHmmss').log"
...
try {
    ...
    # Архивирование
    $archiveName = "run_for_all_${computerName}_v${targetVersion}_$(Get-Date -Format 'yyyyMMddHHmmss').zip"
