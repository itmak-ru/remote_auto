...
$versionHistoryPath = "C:\ProgramData\Remote_Auto\script_history_run_comp_date.json"
...
$targetScriptName = "run_comp_date_${currentDate}_${computerName}.ps1"
...
if (-not $scriptExists) {
    Write-Host "Скрипт на $currentDate для $computerName не найден"
    exit 0
}

# ===== ВЫПОЛНЕНИЕ СКРИПТА =====
$logName = "run_comp_date_${computerName}_${currentDate}_$(Get-Date -Format 'HHmmss').log"
...
try {
    ...
    # Архивирование
    $archiveName = "run_comp_date_${computerName}_${currentDate}_$(Get-Date -Format 'HHmmss').zip"
